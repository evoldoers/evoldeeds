...
def q_bar (idx, nbr_idx, nbr_mask, params, mu):
    ...
    exp_minus_2JC = exp_minus_2J[None,None,:,:] ** nbr_mask[idx,:,None,None]  # (a,k,y_i,x_{nbr_k})
    mu_exp_2JC = jnp.einsum ('akx,kyx->aky', mu[nbr_idx[idx]], exp_minus_2JC)  # (a,k,y_i)
    S = S * offdiag_mask(N)
...
def q_bar_cond (i, nbr_idx, nbr_mask, params, mu):
    ...
    exp_minus_2JC = exp_minus_2J[None,None,:,:] ** nonself_nbr_mask[:,:,None,None]  # (j,k,y_i,x_{nbr_k})
    mu_exp_JC = jnp.einsum ('kx,jkyx->jky', mu[nbr_idx[i]], exp_minus_2JC)  # (j,k,y_i)
    S = S * offdiag_mask(N)
...
def F_deriv (nbr_idx, nbr_mask, params, mu, rho):
    K, N = mu.shape
    idx = jnp.arange(K)
...
def make_rho_term (i):
    def rho_term (t, rho_i_t, args):
        ...
        return rho_deriv (i, nbr_idx, nbr_mask, params, mu, rho)
    return rho_term
...
def make_mu_term (i):
    def mu_term (t, mu_i_t, args):
        ...
        return mu_deriv (i, nbr_idx, nbr_mask, params, mu, rho)
    return mu_term
